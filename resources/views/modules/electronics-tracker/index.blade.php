...
@section('header', $config['name'] . ' — Assets')
@section('content')
<div class="space-y-4">
    <div class="flex justify-between items-center">
        <a href="{{ route('modules.electronics-tracker.dashboard') }}" class="text-sm font-sans text-brand-indigo/70 hover:text-brand-coral transition">&larr; Dashboard</a>
        <a href="{{ route('modules.electronics-tracker.assets.create') }}" class="btn-primary">Add Assets</a>
    </div>
            ...
            <table class="w-full text-sm">
                ...
                @foreach($assets as $item)
                    <tr class="border-b border-brand-lavender/20 hover:bg-brand-lavender/10 transition">
                        ...
                        <td class="py-3 text-right"><a href="{{ route('modules.electronics-tracker.assets.edit', $item) }}" class="font-sans text-brand-coral hover:underline">Edit</a></td>
                    </tr>
                @endforeach
                </tbody>
            </table>
